...
export default function ContactFormClient() {
  ...
  const selectedServiceSummary = useMemo(() => {
    ...
    return SERVICE_SUMMARY_MAP[values.serviceType as ServiceOptionValue]
  }, [values.serviceType])
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmitMessage({
        type: 'success',
        text: '상담 요청이 접수되었습니다. 영업일 기준 24시간 이내에 답변드릴게요.',
      })
    } catch (error) {
      console.error(error)
      setSubmitMessage({
        type: 'error',
        text: '상담 요청 중 문제가 발생했습니다. 잠시 뒤 다시 시도해주세요.',
      })
    } finally {
  ...
  return (
    <>
      <form className={styles.form} noValidate onSubmit={handleSubmit}>
        ...
        <fieldset className={styles.fieldset}>
          ...
          <SelectBox
            ...
            options={[{ label: '서비스를 선택해주세요', value: '' }, ...SERVICE_OPTIONS]}
            background='#1d1d1d'
            textColor='#f3efe5'
            focusBorderColor='#c8f135'
            round='4px'
          ...
          {selectedServiceSummary ? (
            <div className={styles.serviceSummary} aria-live='polite'>
              {selectedServiceSummary.price ? (
                <strong className={styles.serviceSummaryPrice}>{selectedServiceSummary.price}</strong>
              ) : null}
              <p className={styles.serviceSummaryText}>{selectedServiceSummary.summary}</p>
            </div>
          ) : null}
          {errors.serviceType ? (
        ...
        <div className={styles.dateGrid}>
          <fieldset className={styles.fieldset}>
            <FieldLabel htmlFor='deadline' label='희망 마감일' onClick={() => setIsCalendarOpen(true)} />
        ...
        <fieldset className={styles.fieldset}>
          ...
          <SelectBox
            ...
            options={[{ label: '배경 톤을 선택해주세요', value: '' }, ...TONE_OPTIONS]}
            background='#1d1d1d'
            textColor='#f3efe5'
            focusBorderColor='#c8f135'
            round='4px'
        ...
        <fieldset className={styles.fieldset}>
          ...
          <textarea
            ...
            placeholder='프로젝트의 목적과 필수 기능을 자세히 설명해주세요.'
            rows={7}
      ...
      <DateAvailabilityModal
        key={`deadline-${values.deadline}`}
        isOpen={isCalendarOpen}
        value={values.deadline}
        onClose={() => setIsCalendarOpen(false)}
        onApply={(nextValue) => {
          updateField('deadline', nextValue)
