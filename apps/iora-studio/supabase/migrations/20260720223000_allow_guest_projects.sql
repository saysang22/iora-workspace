...
alter table public.projects
  drop constraint if exists projects_member_or_company_required;

alter table public.projects
  add constraint projects_member_or_company_required
  check (
    user_id is not null
    or nullif(btrim(company_name), '') is not null
  );

drop function if exists public.create_project_with_pages(
  uuid,
  text,
...
create or replace function public.create_project_with_pages(
  ...
  input_client_name text default null,
...
begin
  ...
  normalized_company_name := nullif(btrim(input_company_name), '');
  normalized_client_name := nullif(btrim(input_client_name), '');

  if input_user_id is null and normalized_company_name is null then
    raise exception 'Either a linked member or a company name is required.';
  end if;

  if input_user_id is null and normalized_client_name is null then
    raise exception 'Client name is required for guest projects.';
  end if;

  if input_user_id is not null and not exists (
    select 1
